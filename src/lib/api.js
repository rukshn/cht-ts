...
const withSessionCookie = (...args) => {
  const options =
    typeof args[0] === "object" ? Object.assign({}, args[0]) : { url: args[0] };
...
const _request =
  ...
  (...args) => {
    ...
    return retry(() => rpn[method](requestOptions), {
      retries: 5,
      randomize: false,
      factor: 1.5,
    });
...
const logDeprecatedTransitions = (settings) => {
  ...
  if (
    ...
    !Object.keys(appSettings.transitions).length
  ) {
...
const api = {
  getAppSettings: () => {
    ...
    return request.get({ url, json: true }).catch((err) => {
      if (err.statusCode === 404) {
        throw new Error(
          `Failed to fetch existing app_settings from ${url}.\n` +
  ...
  updateAppSettings: (content) => {
    return logDeprecatedTransitions(content)
      .catch((err) => {
        ...
        log.warn(
          "Failed to check for deprecated transitions. Continuing...",
          err,
        );
      })
  ...
  getUserInfo(queryParams) {
    return request.get(`${environment.instanceUrl}/api/v1/users-info`, {
      qs: queryParams,
      json: true,
    });
  },
  ...
  available: async () => {
    ...
    try {
      ...
      if (err.statusCode === 401) {
        throw new Error(
          `Authentication failed connecting to ${url}. ` +
            "Check the supplied username and password and try again.",
        );
      }
      if (err.statusCode === 403) {
        throw new Error(
          `Insufficient permissions connecting to ${url}. ` +
            "You need to use admin permissions to execute this command.",
        );
      }
      if (err.statusCode) {
        throw new Error(
          `Received error code ${err.statusCode} connecting to ${url}. ` +
            "Check the server and and try again.",
        );
      }
      throw new Error(
        `Failed to get a response from ${url}. Maybe you entered the wrong URL, ` +
  ...
  version() {
    return request
      .get({ uri: `${environment.instanceUrl}/api/deploy-info`, json: true }) // endpoint added in 3.5
    ...
    return request
      ...
      .then((resp) => {
        try {
          ...
          throw new Error(
            "Invalid JSON response validating XForm against the API: " + resp,
          );
        }
      })
      .catch((err) => {
  ...
  async getCompressibleTypes() {
    ...
    try {
      ...
      const compressibleTypes = resp.compressible_types
        .split(",")
        .map((s) => s.trim());
