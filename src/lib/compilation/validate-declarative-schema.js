  ...
  .items(
    joi.object({
      ...
      appliesIf: joi
        ...
        .error(
          targetError(
            '"appliesIf" should be of type function(contact, report)',
          ),
        ),
      passesIf: joi
        .alternatives()
      ...
      groupBy: joi
        ...
        .optional()
        .error(
          targetError('"groupBy" should be of type function(contact, report)'),
        ),
      ...
      date: joi
        .alternatives()
        .try(joi.string().valid("reported", "now"), joi.function())
        .optional()
        .error(
          targetError(
      ...
      dhis: joi
        .alternatives()
        .try(DhisSchema, joi.array().items(DhisSchema))
        .optional(),
      visible: joi.boolean().optional(),
      idType: joi
        .alternatives()
        .try(joi.string().valid("report", "contact"), joi.function())
        .optional()
        .error(
          targetError(
            'idType should be either "report" or "contact" or "function(contact, report)"',
          ),
        ),
...
const EventSchema = (idPresence) =>
  joi.object({
    ...
    days: joi
      ...
      .error(
        taskError(
          '"event.days" is a required integer field only when "event.dueDate" is absent',
        ),
      ),
    dueDate: joi
      .alternatives()
...
const TaskSchema = joi
  ...
  .items(
    joi.object({
      ...
      appliesTo: joi
        .string()
        .valid("contacts", "reports", "scheduled_tasks")
        .required(),
      appliesIf: joi
        .function()
        .optional()
        .error(
          taskError('"appliesIf" should be of type function(contact, report)'),
        ),
      appliesToType: joi.array().items(joi.string()).optional().min(1),
      contactLabel: joi
        .alternatives()
        .try(joi.string().min(1), joi.function())
        .optional()
        .error(
          taskError(
            '"contactLabel" should either be a non-empty string or function(contact, report)',
          ),
        ),
      resolvedIf: joi
        .alternatives()
      ...
      actions: joi
        ...
        .items(
          joi.object({
            ...
            form: joi.alternatives().conditional("type", {
              is: "contact",
              then: joi.forbidden(),
            ...
            modifyContent: joi
              ...
              .error(
                taskError(
                  '"actions.modifyContent" should be "function (content, contact, report)',
                ),
              ),
          }),
        )
...
module.exports = (projectDir, errorOnValidation) => {
  ...
  const targetsValid = validateFile(
    logEvent,
    projectDir,
    "targets.js",
    TargetSchema,
